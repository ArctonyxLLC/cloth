...
class Pool extends EventEmitter {
  ...
  _spawn () {
    ...
    child.on('error', function(err) {
      ...
      console.error('Got an error from worker %s: %s', child.pid, err);
      cleanUpChild(err);
    });
  ...
  _allocate (pid) {
    ...
    if (this.options.maxTasksPerWorker > 0 && child._taskCount_ >= this.options.maxTasksPerWorker) {
      console.debug('Stopping worker %s after it completed %s tasks', child.pid, child._taskCount_);
      child.kill();
    }
    else if (this.tasks.length) {
      this.emit('empty');
